...
from ngclearn.museum.gncn_pdh_classify import GNCN_PDH_Classify
...
Usage:
$ python sim_train.py --config=pdh-fit.cfg --gpu_id=0
...
with tf.device(gpu_tag):
    def calc_ToD(agent):
        ...
        ToD = 0.0
        L3 = agent.ngc_model.extract(node_name="e3", node_var_name="L")
    ...
    for trial in range(n_trials): # for each trial
        ...
        agent = GNCN_PDH_Classify(args) # set up NGC model
        ...
        for i in range(num_iter): # for each training iteration/epoch
            ...
            for batch in train_set:
                ...
                mu3 = agent.ngc_sampler.extract("s3","z") # extract value of s3
                ...
                agent.ngc_model.inject([("mu3", "z", mu3), ("z3", "z", mu3),
                                        ("mu2", "z", mu2), ("z2", "z", mu2),
                                        ("mu1", "z", mu1), ("z1", "z", mu1),
                                        ("mu0", "z", mu0)])
