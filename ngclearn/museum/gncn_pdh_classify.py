...
class GNCN_PDH_Classify:
    ...
    def __init__(self, args):
        ...
        z4 = SNode(name="z4", dim=x_dim, beta=beta, leak=leak, act_fx="identity",
                   integrate_kernel=integrate_cfg)
        z3 = SNode(name="z3", dim=z_dim, beta=beta, leak=leak, act_fx=act_fx,
                   integrate_kernel=integrate_cfg)
        ...
        mu3 = SNode(name="mu3", dim=z_dim, act_fx="identity", zeta=0.0)
        ...
        e3 = ENode(name="e3", dim=z_dim) #, precis_kernel=precis_cfg)
        e2 = ENode(name="e2", dim=z_dim) #, precis_kernel=precis_cfg)
        e1 = ENode(name="e1", dim=z_dim) #, precis_kernel=precis_cfg)
        e0 = ENode(name="e0", dim=y_dim) #, precis_kernel=precis_cfg)
        ...
        z4_mu3 = z4.wire_to(mu3, src_comp="phi(z)", dest_comp="dz_td", cable_kernel=dcable_cfg,
                            short_name="W4")
        ...
        z1_mu0 = z1.wire_to(mu0, src_comp="phi(z)", dest_comp="dz_td", cable_kernel=dcable_cfg,
                            short_name="W1")
        z4_mu3.set_constraint(constraint_cfg)
        z3_mu2.set_constraint(constraint_cfg)
        z2_mu1.set_constraint(constraint_cfg)
        z1_mu0.set_constraint(constraint_cfg)

        e3_z4 = e3.wire_to(z4, src_comp="phi(z)", dest_comp="dz_bu", cable_kernel=ecable_cfg,
                           short_name="E4")
        ...
        e0_z1 = e0.wire_to(z1, src_comp="phi(z)", dest_comp="dz_bu", cable_kernel=ecable_cfg,
                           short_name="E1")
        e3_z4.set_constraint(constraint_cfg)
        e2_z3.set_constraint(constraint_cfg)
        e1_z2.set_constraint(constraint_cfg)
        e0_z1.set_constraint(constraint_cfg)

        z4_mu2 = z4.wire_to(mu2, src_comp="phi(z)", dest_comp="dz_td", cable_kernel=dcable_cfg,
                            short_name="S4")
        ...
        z2_mu0 = z2.wire_to(mu0, src_comp="phi(z)", dest_comp="dz_td", cable_kernel=dcable_cfg,
                            short_name="S2")
        z4_mu2.set_constraint(constraint_cfg)
        z3_mu1.set_constraint(constraint_cfg)
        z2_mu0.set_constraint(constraint_cfg)

        mu3.wire_to(e3, src_comp="phi(z)", dest_comp="pred_mu", cable_kernel=pos_scable_cfg,
                    short_name="1")
        z3.wire_to(e3, src_comp="phi(z)", dest_comp="pred_targ", cable_kernel=pos_scable_cfg,
                   short_name="1")
        e3.wire_to(z3, src_comp="phi(z)", dest_comp="dz_td", cable_kernel=neg_scable_cfg,
                   short_name="-1")
        ...
        z4_mu3.set_update_rule(preact=(z4,"phi(z)"), postact=(e3,"phi(z)"), param=["A"])
        ...
        z4_mu2.set_update_rule(preact=(z4,"phi(z)"), postact=(e2,"phi(z)"), param=["A"])
        ...
        e_gamma = 1.0
        e3_z4.set_update_rule(preact=(e3,"phi(z)"), postact=(z4,"phi(z)"), gamma=e_gamma, param=["A"])
        ...
        ngc_model.set_cycle(nodes=[z4, z3, z2, z1, z0])
        ngc_model.set_cycle(nodes=[mu3, mu2, mu1, mu0])
        ngc_model.set_cycle(nodes=[e3, e2, e1, e0])
        ...
        z4_dim = ngc_model.getNode("z4").dim
        ...
        s4 = FNode(name="s4", dim=z4_dim, act_fx="identity")
        ...
        s4_s3 = s4.wire_to(s3, src_comp="phi(z)", dest_comp="dz", mirror_path_kernel=(z4_mu3,"A"))
        ...
        s4_s2 = s4.wire_to(s2, src_comp="phi(z)", dest_comp="dz", mirror_path_kernel=(z4_mu2,"A"))
        ...
        sampler.set_cycle(nodes=[s4, s3, s2, s1, s0])
    ...
    def project(self, z_sample):
        ...
        readouts = self.ngc_sampler.project(
                        clamped_vars=[("s4","z",tf.cast(z_sample,dtype=tf.float32))],
    ...
    def settle(self, x, y, calc_update=True):
        ...
        readouts, delta = self.ngc_model.settle(
                            clamped_vars=[("z4","z",x),("z0","z",y)],
                            readout_vars=[("mu0","phi(z)"),("mu1","phi(z)"),("mu2","phi(z)"), ("mu3","phi(z)")],
